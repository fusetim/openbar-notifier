...
pub trait ItemsApi: Send + Sync {
    ...
    async fn get_category_items<'category_id, 'page, 'limit, 'state>(
        &self,
        category_id: &str,
        page: Option<i64>,
        limit: Option<i64>,
        state: Option<&'state str>,
    ) -> Result<models::GetCategoryItems200Response, Error<GetCategoryItemsError>>;

    /// GET /categories/{category_id}/items/{item_id}/picture
    ///
    /// Get an item picture
    async fn get_item_picture<'category_id, 'item_id>(
        &self,
        category_id: &str,
        item_id: &str,
    ) -> Result<std::path::PathBuf, Error<GetItemPictureError>>;
}
...
impl ItemsApi for ItemsApiClient {
    ...
    async fn get_category_items<'category_id, 'page, 'limit, 'state>(
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<GetCategoryItemsError> =
    ...
    async fn get_item_picture<'category_id, 'item_id>(
        ...
        let local_var_uri_str = format!(
            ...
            local_var_configuration.base_path,
            category_id = crate::apis::urlencode(category_id),
            item_id = crate::apis::urlencode(item_id)
        );
        let mut local_var_req_builder =
            local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<GetItemPictureError> =
                serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent {
                status: local_var_status,
                content: local_var_content,
