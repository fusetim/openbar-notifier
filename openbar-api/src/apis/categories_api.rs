...
pub trait CategoriesApi: Send + Sync {
    ...
    ///
    /// Get all categories
    async fn get_categories<'hidden>(
        &self,
        hidden: Option<bool>,
    ) -> Result<Vec<models::Category>, Error<GetCategoriesError>>;

    /// GET /categories/{category_id}
    ///
    /// Get a category
    async fn get_category<'category_id>(
        &self,
        category_id: &str,
    ) -> Result<models::Category, Error<GetCategoryError>>;

    /// GET /categories/{category_id}/picture
    ///
    /// Get a category picture
    async fn get_category_picture<'category_id>(
        &self,
        category_id: &str,
    ) -> Result<std::path::PathBuf, Error<GetCategoryPictureError>>;
}
...
impl CategoriesApi for CategoriesApiClient {
    ...
    async fn get_categories<'hidden>(
        ...
        if let Some(ref param_value) = hidden {
            local_var_req_builder =
                local_var_req_builder.query(&[("hidden", &param_value.to_string())]);
        }
    ...
    async fn get_category<'category_id>(
        ...
        let local_var_uri_str = format!(
            ...
            category_id = crate::apis::urlencode(category_id)
        );
        let mut local_var_req_builder =
            local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    async fn get_category_picture<'category_id>(
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<GetCategoryPictureError> =
                serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent {
                status: local_var_status,
                content: local_var_content,
