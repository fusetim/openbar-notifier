...
pub trait CategoriesApi: Send + Sync {
    ...
    ///
    /// Get all categories
    async fn get_categories<'hidden>(&self, hidden: Option<bool>) -> Result<Vec<models::Category>, Error<GetCategoriesError>>;

    /// GET /categories/{category_id}
    ///
    /// Get a category
    async fn get_category<'category_id>(&self, category_id: &str) -> Result<models::Category, Error<GetCategoryError>>;

    /// GET /categories/{category_id}/picture
    ///
    /// Get a category picture
    async fn get_category_picture<'category_id>(&self, category_id: &str) -> Result<std::path::PathBuf, Error<GetCategoryPictureError>>;
}
...
impl CategoriesApi for CategoriesApiClient {
    ...
    async fn get_categories<'hidden>(&self, hidden: Option<bool>) -> Result<Vec<models::Category>, Error<GetCategoriesError>> {
        ...
        if let Some(ref param_value) = hidden {
            local_var_req_builder = local_var_req_builder.query(&[("hidden", &param_value.to_string())]);
        }
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            match local_var_content_type {
                ...
                ContentType::Unsupported(local_var_unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{local_var_unknown_type}` content type response that cannot be converted to `Vec&lt;models::Category&gt;`")))),
            }
        } else {
    ...
    async fn get_category<'category_id>(&self, category_id: &str) -> Result<models::Category, Error<GetCategoryError>> {
        ...
        let local_var_uri_str = format!("{}/categories/{category_id}", local_var_configuration.base_path, category_id=crate::apis::urlencode(category_id));
        let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    async fn get_category_picture<'category_id>(&self, category_id: &str) -> Result<std::path::PathBuf, Error<GetCategoryPictureError>> {
        ...
        let local_var_uri_str = format!("{}/categories/{category_id}/picture", local_var_configuration.base_path, category_id=crate::apis::urlencode(category_id));
        let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<GetCategoryPictureError> = serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
            Err(Error::ResponseError(local_var_error))
        }
    }
...
/// struct for typed errors of method [`get_category_picture`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetCategoryPictureError {
    Status404(models::HttpError),
    Status500(models::HttpError),
    UnknownValue(serde_json::Value),
}
